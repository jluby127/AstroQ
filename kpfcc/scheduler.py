...
class Scheduler(object):
    ...
    def __init__(self, manager, request_set):
    ...
    def constraint_one_request_per_slot(self):
        ...
        Aframe_ds = pd.merge(
            self.request_set.Aframe.drop_duplicates(['d', 's']),
            self.request_set.Aframe[['r', 'd', 's']],
            suffixes=['', '2'],
            on=['d', 's'])
        ds_requests = Aframe_ds.groupby(['d','s'])[['r2']].agg(list)
        Aset_ds_no_duplicates = self.request_set.Aframe.copy()
        Aset_ds_no_duplicates = self.request_set.Aframe.drop_duplicates(subset=['d', 's'])
    ...
    def constraint_reserve_multislot_exposures(self):
        ...
        requires_multislot = self.request_set.Aframe.e > 1
        ...
        Aframe_multislots = self.request_set.Aframe[requires_multislot]
    ...
    def constraint_max_visits_per_night(self):
        ...
        aframe_slots_on_day = pd.merge(
            self.request_set.Aframe.drop_duplicates(['r','d',]),
            self.request_set.Aframe[['r','d','s']],
            suffixes=['','2'],on=['r']
        ).query('d == d2')
        slots_on_day = aframe_slots_on_day.groupby(['r','d'])[['s2']].agg(list)
        self.slots_on_day = slots_on_day
        unique_request_day_pairs = self.request_set.Aframe.drop_duplicates(['r','d'])
    ...
    def constraint_enforce_internight_cadence(self):
        ...
        mask_inter_1 = self.request_set.Aframe['ter'] > 1
    ...
    def build_model_round1(self):
        ...
        self.model.constraint_one_request_per_slot()
        self.model.constraint_reserve_multislot_exposures()
        self.model.constraint_enforce_internight_cadence()

        self.model.constraint_set_max_desired_unique_nights_Wrd()
        self.model.constraint_build_enforce_intranight_cadence()
        self.model.constraint_set_min_max_visits_per_night()
        self.model.constraint_build_theta_multivisit()

        if self.manager.run_optimal_allocation:
            self.model.constraint_set_max_quarters_allocated()
            self.model.constraint_set_max_onsky_allocated()
            self.model.constraint_relate_allocation_and_onsky()
            self.model.constraint_all_portions_of_night_represented()
            self.model.constraint_forbidden_quarter_patterns()
            self.model.constraint_cannot_observe_if_not_allocated(self.manager.twilight_map_remaining_2D)
            if os.path.exists(self.manager.blackout_file):
                self.model.constraint_enforce_restricted_nights(limit=0)
            if os.path.exists(self.manager.whiteout_file):
                self.model.constraint_enforce_restricted_nights(limit=1)
            if manager.include_aesthetic:
                self.model.constraint_max_consecutive_onsky()
                self.model.constraint_minimum_consecutive_offsky()
                self.model.constraint_maximize_baseline()

        self.model.model.set_objective_minimize_theta_time_normalized()
        print("Time to build constraints: ", np.round(time.time()-t1,3))

    def build_model_round2(self):
        t1 = time.time()
        self.model.remove_constraint_set_max_desired_unique_nights_Wrd()
        self.model.constraint_set_max_absolute_unique_nights_Wrd()
        self.model.constraint_fix_previous_objective()
        self.model.set_objective_maximize_slots_used()
        print("Time to build constraints: ", np.round(time.time()-t1,3))
