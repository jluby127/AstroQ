...
class RequestSet(object):
    ...
    def read_from_json(self, filename):
        ...
        # self.Aset = np.array([tuple(x) for x in Aset_frame[['id', 'd', 's']].values])
        self.Aset = Aset_frame

        unique_d_for_each_id = self.Aset.groupby('id')['d'].unique()
        self.schedulable_requests = list(unique_d_for_each_id.keys())
        Wset = []
        for starname in self.schedulable_requests:
