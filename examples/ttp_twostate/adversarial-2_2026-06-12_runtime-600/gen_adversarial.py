"""Generate a staggered adversarial 58-target night (rise/set timing version).

Earlier the targets were all up most of the night, so the solver could observe
every southern target first and cross the legacy az=235 cut only once. Here we
stagger availability via RA so that the "all-south-first" order is infeasible:

Two dec buckets are kept (29 south ``dec`` in [-20,10] -> transit az ~180;
29 NW ``dec`` in [22,38] -> set az ~300). Within each bucket targets are split
by timing class (even split, per the agreed design):

- early-setting (~7/bucket): up at dusk, drops below access before t0 + 0.25 L
  (sets in the west, az > 235).
- late-rising  (~7/bucket): not accessible until after t0 + 0.75 L
  (rises in the east, az < 235).
...
Usage:
    python examples/ttp_twostate/adversarial-2_2026-06-12_runtime-600/gen_adversarial.py
"""
...
def build():
    ...
    rows = []
    k = 0
    for bucket, decs in (("S", SOUTH_DECS), ("NW", NW_DECS)):
