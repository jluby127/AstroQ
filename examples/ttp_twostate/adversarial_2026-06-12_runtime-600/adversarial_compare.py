"""Single- vs two-state TTP on the adversarial south<->NW target set.

Unlike the production demo, requests here carry explicit tight ``window_start_ut``
/ ``window_end_ut`` columns. We build each target's ``first/last_available`` as
that window intersected with real accessibility (alt in [18,85], outside the
Nasmyth zone), so the staggered windows actually constrain the visit order.
...
modeled slew, physical slew, MIP gap, and a count of sky-az 235 crossings (the
legacy-expensive south<->NW transitions).

Usage (astroq-testing env, from repo root):
    python examples/ttp_twostate/adversarial_2026-06-12_runtime-600/adversarial_compare.py        # both
    python examples/ttp_twostate/adversarial_2026-06-12_runtime-600/adversarial_compare.py 2      # two-state only
"""
...
def build_requests_access(df, queue, night_start, night_end, *, grid_min=2.0):
    ...
    The synthetic ``window_*_ut`` columns are intentionally ignored here: the
    point is to let real observability (not artificial tight windows) define the
    feasible region, so the solver actually optimizes the visit order.
    """
...
def write_summary(results):
    ...
    lines = [
        "Adversarial south<->NW TTP comparison (forces az 235 crossings)",
        ...
        "",
        f"{'model':<14}{'sched':>7}{'phys slew':>11}{'mod slew':>10}{'az235x':>8}{'gap':>9}{'solve s':>9}",
        "-" * 68,
    ]
    ...
    lines += [
        ...
        "'az235x' counts scheduled transitions straddling sky az 235.",
    ]
    txt = "\n".join(lines)
