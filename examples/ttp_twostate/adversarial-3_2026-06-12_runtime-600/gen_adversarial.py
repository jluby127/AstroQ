"""Generate a staggered adversarial 58-target night (v3: wider dec spread).

Same staggered rise/set design as v2, but the two dec buckets are spread out
much more in declination so the per-slew costs are more realistic (a slew
between two targets now depends meaningfully on their dec separation rather
than every target sitting at nearly the same dec):

- South bucket: 29 targets with ``dec`` in [-40, -20] -> low southern transit.
- North bucket: 29 targets with ``dec`` in [40, 60] -> high northern transit.
...
~10 min single-shot visit (``exptime = 600 s``).

Usage:
    python examples/ttp_twostate/adversarial-3_2026-06-12_runtime-600/gen_adversarial.py
"""
...
# v3: spread the buckets out in declination for realistic per-slew costs.
SOUTH_DECS = np.linspace(-40.0, -20.0, N_PER)
NORTH_DECS = np.linspace(40.0, 60.0, N_PER)
