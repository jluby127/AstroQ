...
sed -i "" "${line_number}s|.*|$new_content|" "bench/config_benchmark.ini"
line_number=7
new_content="current_day = 2018-08-01"
sed -i "" "${line_number}s|.*|$new_content|" "bench/config_benchmark.ini"
...
do
  ...
  astroq bench -ns $ns_val -cf bench/config_benchmark.ini > $outdir$ns_val/logfile-$ns_val.log
...
do
  ...
  astroq bench -ns 12 -cf bench/config_benchmark.ini > $outdir/random_$number/logfile-$number.log
...
do
  ...
  astroq bench -ns 12 -cf bench/config_benchmark.ini > $outdir/everynight_$number/logfile-$number.log
...
do
  ...
  sed -i "" "${line_number}s|.*|$new_content|" "bench/config_benchmark.ini"
  astroq bench -ns 12 -cf bench/config_benchmark.ini > $outdir/weath_$run_numb/logfile-$run_numb.log
