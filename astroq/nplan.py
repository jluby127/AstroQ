...
def _restore_requests_frame_legacy(thin_rf, *, night_planner_instance):
    ...
    if os.path.exists(selected_path):
        ...
        try:
            ns, ne = get_nightly_times_from_allocation(
                instance.allocation_file,
                instance.current_day,
            )
    ...
    selected_df["n_intra_max"] = (
        selected_df["n_intra_max"].replace("None", np.nan).fillna(1)
    )
    ...
    out["tau_intra"] = (
        selected_df["tau_intra"].replace("None", np.nan).fillna(0.0).to_numpy() * u.hr
    )
...
class NightPlanner(object):
    ...
    def __init__(self, config_file):
        ...
        self.slots_needed_for_exposure_dict = (
            self.semester_planner.slots_needed_for_exposure_dict
        )
        ...
        tonight_allocated = access_record.is_alloc[
            0, tonight_index, :
        ]  # 1D: 1=allocated, 0=not
        ...
        gap_start_slots = (
            np.where(diff == -1)[0] + 1
        )  # first zero slot of each potential gap
        gap_end_slots = np.where(diff == 1)[
            0
        ]  # last zero slot before each 0->1 transition
        ...
        for start_slot in gap_start_slots:
            ...
            if len(candidates) > 0:
                ...
                gap_start_time = (
                    f"{int(start_minutes // 60):02d}:{int(start_minutes % 60):02d}"
                )
                gap_stop_time = (
                    f"{int(end_minutes // 60):02d}:{int(end_minutes % 60):02d}"
                )
        ...
        self.tonight_total_unallocated_minutes = (
            self.tonight_total_unallocated_slots * slot_size
        )
        self.tonight_gap_unallocated_slots = sum(
            g["n_slots_in_gap"] for g in self.tonight_allocation_gaps
        )
        self.tonight_gap_unallocated_minutes = (
            self.tonight_gap_unallocated_slots * slot_size
        )
    ...
    def run_ttp(self):
        ...
        try:
            observation_start_time, observation_stop_time = (
                get_nightly_times_from_allocation(
                    self.allocation_file, self.current_day
                )
            )
            total_time = np.round(
                (observation_stop_time.jd - observation_start_time.jd) * 24, 3
            )
            print("Time in Night for Observations: " + str(total_time) + " hours.")
        except ValueError:
            print(
                f"No allocation times found for date {self.current_day}. Not running TTP. No night_planner.h5 file will be created."
            )
            return False
        ...
        if not os.path.exists(selected_path):
            raise FileNotFoundError(
                f"{selected_path} not found. Please run the scheduler first."
            )
        selected_df = pd.read_csv(selected_path)
        if len(selected_df) == 0:
            print(
                f"No targets found in {selected_path}. Not running TTP. No night_planner.pkl file will be created."
            )
            return
        ...
        if "n_intra_min" in selected_df.columns:
            selected_df["n_intra_min"] = (
                selected_df["n_intra_min"].replace("None", np.nan).fillna(1)
            )
        selected_df["tau_intra"] = (
            selected_df["tau_intra"].replace("None", np.nan).fillna(0.0)
        )
        ...
        if len(self.tonight_allocation_gaps) > 0:
            ...
            for i, gap in enumerate(self.tonight_allocation_gaps, start=1):
                ...
                exposure_time_sec = (
                    gap["gap_length"] - self.semester_planner.slot_size
                ) * 60
                gap_rows.append(
                    {
                        "unique_id": f"Gap {i}",
                        "coord": SkyCoord(
                            avg_ra * u.deg, avg_dec * u.deg, frame="icrs"
                        ),
        ...
        gap_exposure_min = (
            float(gap_scheduled["t_visit"].sum()) if len(gap_scheduled) else 0.0
        )
        ...
        if gap_total_min > 0 or gap_exposure_min > 0:
            ...
            tm.stats.update(
                model.TTPModel._exposure_timing_stats(
                    real_scheduled,
                    tm.stats["dur"],
                )
            )
        ...
        for _, row in scheduled_df.iterrows():
            adjusted_timestamp = (
                TimeDelta(row["t_start"] * 60, format="sec") + observation_start_time
            )
    ...
    def get_first_last_indices(self, selected_df):
        ...
        for _, row in selected_df.iterrows():
            ...
            if target_id in target_to_index:
                ...
                tonight_observable = access_record.is_observable[
                ...
                if len(true_indices) > 0:
                    ...
                    first_available.append(
                        f"{self.current_day} {first_hour:02d}:{first_minute:02d}"
                    )
                    last_available.append(
                        f"{self.current_day} {last_hour:02d}:{last_minute:02d}"
                    )
                else:
    ...
    def to_hdf5(self, hdf5_path=None):
        ...
        if rf.empty:
            rf.to_hdf(
                hdf5_path, key="solution_requests_frame", mode="a", format="fixed"
            )
        else:
            rf.to_hdf(
                hdf5_path, key="solution_requests_frame", mode="a", format="table"
            )
        ...
        with h5py.File(hdf5_path, "a") as f:
            ...
            semester_planner_h5_path = os.path.join(
                self.output_directory, "semester_planner.h5"
            )
    ...
    def from_hdf5(cls, hdf5_path):
        ...
        with h5py.File(hdf5_path, "r") as f:
            ...
            instance.semester_planner = SemesterPlanner.from_hdf5(
                semester_planner_h5_path
            )
            ...
            instance.today_starting_night = (
                instance.semester_planner.today_starting_night
            )
            instance.past_history = instance.semester_planner.past_history
            instance.slots_needed_for_exposure_dict = (
                instance.semester_planner.slots_needed_for_exposure_dict
            )
            ...
            for hdf5_key, attr_name, data_type, extra in solution_attrs:
                if data_type == "dict_json":
                    ...
                    times_jd = f[hdf5_key][:]
                    setattr(
                        solution, attr_name, [Time(jd, format="jd") for jd in times_jd]
                    )
        ...
        if (
            solution.requests_frame is not None
            and "unique_id" in solution.requests_frame.columns
        ):
