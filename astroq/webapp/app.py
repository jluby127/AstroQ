...
def render_admin_page(semester_code, date, band):
    ...
    fig_cof2 = pl.get_cof(
        semester_planner, list(data_astroq[1].values()), use_time=True
    )
    ...
    fig_timebar_by_program_html = pio.to_html(
        fig_timebar_by_program, full_html=True, include_plotlyjs="cdn"
    )
...
def render_program_page(semester_code, date, band, program_code):
    ...
    fig_timebar = pl.get_timebar(
        semester_planner, program_stars, use_program_colors=True
    )
...
def render_star_page(starname, program_code=None):
    ...
    programs_to_search = (
        [program_code]
        if program_code and program_code in data_astroq[0]
        else data_astroq[0].keys()
    )
    ...
    for program in programs_to_search:
        for star_ind in range(len(data_astroq[0][program])):
            ...
            if object_compare_starname == compare_starname:
                ...
                fig_cof = pl.get_cof(
                    semester_planner, [data_astroq[0][program][star_ind]]
                )
                fig_birdseye = pl.get_birdseye(
                ...
                fig_rawobs_html = pio.to_html(
                    fig_rawobs, full_html=True, include_plotlyjs="cdn"
                )
...
def render_nightplan_page(band):
    ...
    script_table_html = pl.nightplan_table_to_html(
...
def download_nightplan(semester_code, date, band):
    ...
    try:
        ...
        script_file_path = os.path.join(
            semester_planner.output_directory,
