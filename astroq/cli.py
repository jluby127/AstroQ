...
parser = argparse.ArgumentParser(
    description="AstroQ: Optimized observation scheduling", prog="astroq"
)
...
def main():
    ...
    psr_bench = subpsr.add_parser(
        "bench",
        parents=[psr_parent],
        description="Conduct benchmark tests",
        prefix_chars="-",
    )
    psr_bench.add_argument(
        "-ns",
        "--number_slots",
        type=int,
        required=True,
    ...
    prep_subpsr = psr_prep.add_subparsers(
        title="prep subcommands", dest="prep_subcommand", required=True
    )

    ## subcommand of prep: kpfcc -- Prep for a new semester for KPF-CC
    psr_prep_kpfcc = prep_subpsr.add_parser(
        "kpfcc",
        description="Compile and prepare all necessary files for the KPF-CC program.",
        prefix_chars="-",
    )
    psr_prep_kpfcc.add_argument(
        "-cf",
        "--config_file",
        type=str,
        required=True,
    ...
    ## subcommand of prep: hirescps -- Prep for a new HIRES CPS night
    psr_prep_hirescps = prep_subpsr.add_parser(
        "hirescps",
        description="Compile and prepare all necessary files for the HIRES CPS queue program.",
        prefix_chars="-",
    )
    ...
    psr_prep_hirescps.set_defaults(func=astroq.driver.hirescps_prep)

    ## subcommand of astroq: webapp -- launch web app to view interactive plots
    psr_webapp = subpsr.add_parser(
        "webapp",
        parents=[psr_parent],
    ...
    psr_plan_semester = subpsr.add_parser(
        "plan-semester",
        parents=[psr_parent],
