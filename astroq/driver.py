...
def bench(args):
    ...
    print(
        f"Request frame thinned: {original_size} -> {new_size} rows (factor of {thin})"
    )
...
def hirescps_prep(args):
    ...
    if allo_source == "db":
        ...
        allocation_frame, hours_by_program, nights_by_program = (
            kpfcc.pull_allocation_info(start_date, n_days, "HIRES", conversion_ratio)
        )
        awarded_programs = [
            semester + "_" + val for val in list(hours_by_program.keys())
        ]
        programmatics = pd.DataFrame(
            {
        ...
        allocation_frame, hours_by_program, nights_by_program = (
            kpfcc.format_keck_allocation_info(allo_source)
        )
        awarded_programs = [
            semester + "_" + val for val in list(hours_by_program.keys())
        ]
...
def kpfcc_prep(args):
    ...
    if allo_source == "db":
        ...
        conversion_ratio = config.getfloat("semester", "hours_per_night")
        allocation_frame, hours_by_program, nights_by_program = (
            kpfcc.pull_allocation_info(start_date, n_days, "KPF-CC", conversion_ratio)
        )
        awarded_programs = [
            semester + "_" + val for val in list(hours_by_program.keys())
        ]
    ...
    if args.request_source == "db":
        ...
        for i in range(len(bad_obs_values)):
            if bad_obs_values["metadata.semid"][i] in awarded_programs:
                send_emails_with.append(
                    kpfcc.inspect_row(bad_obs_hasFields, bad_obs_values, i)
                )
        """
...
def plot(args):
    ...
    if os.path.exists(
        ...
        semester_planner = splan.SemesterPlanner.from_hdf5(
            os.path.join(semester_directory, "outputs", "semester_planner.h5")
        )
        ...
        print(
            f"No semester_planner.h5 found in {semester_directory}/outputs/. No plots will be generated."
        )
