...
the_schedule = np.loadtxt(savepath + 'raw_combined_semester_schedule.txt', delimiter=',', dtype=str)
for n in range(len(args.schedule_dates)):
    ...
    toTTP = pf.prepareTTP(request_sheet, the_schedule[dayInSemester])
    ...
    pf.write_starlist(all_targets_frame, obs_and_times, 'nominal', str(args.schedule_dates[0]), savepath)
